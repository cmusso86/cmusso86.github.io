...
tab1 <- flextable (obitos_Alemanha_Franca_resumo) %>% 
  autofit() %>%  # essa é uma função nova %>%  ##Ajusta automaticamente as larguras das colunas
  align(align="left", j=1, part="all") %>%  ##Alinha a primeira coluna à esquerda
  align(align="right", j=c(2,3), part="all") %>% ##Alinha as colunas 2 e 3 à direita
  bold(j=1, part="all") %>% ##Negrito na primeira coluna
  bold(part="header") %>% ##Negrito no cabeçalho
  set_header_labels(      ##Definindo o nome das variáveis
    media_obitos_diarios="Média óbitos \n Diários",
    maximo_obitos_acumulados= "Óbitos acumulados") %>% 
  add_header_row(values = c("","Indicadores"), colwidth=c(1,2)) %>% ##Cabeçalho extra
  align(align="c", part="all") %>% 
  save_as_docx(path="tabela1.docx") ##salvando o documento em um documento de texto
...
tab1 <- flextable (obitos_Alemanha_Franca_resumo) %>% 
  ...
  bold(j=1, part="all") %>%
  bold(part="header") %>%
  font(part = "header", fontname = "Times") %>% ## Adicionando essa linha, podemo alterar a fonte.
  set_header_labels(
    media_obitos_diarios="Média óbitos \n Diários",
    maximo_obitos_acumulados= "Óbitos acumulados") %>% 
...
set_caption(tab1, titulo) ##Precisamos inserir o objeto correspondente à tabela (no caso, tab1) e o título que queremos inserir (pode ser em texto ou um objeto). É necessário que a tabela esteja como objeto flextable.
