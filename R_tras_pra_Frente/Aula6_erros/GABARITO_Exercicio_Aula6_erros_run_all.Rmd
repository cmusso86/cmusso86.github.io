...
ggplot(who_trat)+
  geom_bar(aes(x=Country, y=Acumulado), stat="identity", 
           color="black", fill="orange")+     #### Olha um erro aqui! A palavra Black deve estar entre aspas, já que não é um objeto e sim um parâmetro do argumento da função.
  coord_flip()+
  theme_classic()
...
Tab1<-who_trat %>%      #### Antes de "Tab1" tínhamos um parêntese sendo aberto sem motivo. Podemos eliminá-lo
  flextable() %>% 
  width(width=2) %>% 
  ...
  bg(bg="orange", part="header")     #### Dois erros em uma linha: Tínhamos um fechamento de parêntese desnecessário e uma aspa que foi aberta mas não foi fechada.
...
# Tabela no documento 
  ...
  set_caption(caption = titulo_1, autonum = run_autonum()) %>%
    autofit()     #### Um erro bastante simples e bastane comum: Esquecer de fechar o parêntese. Todo parentese e aspa que abrimos, precisamos lembrar de fechar!
